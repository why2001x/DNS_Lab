...
int RecordSortComp(void* LPtr, void* RPtr) {
    ...
    if (Comp) return Comp;
    if (Lhs.Type != Rhs.Type) return (Lhs.Type > Rhs.Type) - (Lhs.Type < Rhs.Type);
    switch ((enum QueryType)Lhs.Type) {
    case A:    return ipv4Comp(Lhs.Data.IPv4, Rhs.Data.IPv4);
    case AAAA: return ipv6Comp(Lhs.Data.IPv6, Rhs.Data.IPv6);
    default:   return 0;
    }
}
int RecordFindComp(void* KeyPtr, void* ParPtr) {
    ...
    if (Comp) return Comp;
    if (Key.Type != Par.Type) return (Key.Type > Par.Type) - (Key.Type < Par.Type);
    switch ((enum QueryType)Key.Type) {
    case A:
        if (ipv4Comp(Key.Data.IPv4, RECORD_DATA_MAX.IPv4) == 0) return 0;
        return ipv4Comp(Key.Data.IPv4, Par.Data.IPv4);
    case AAAA:
        if (ipv6Comp(Key.Data.IPv6, RECORD_DATA_MAX.IPv6) == 0) return 0;
